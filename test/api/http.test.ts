...
describe("HttpResourceManager (integration)", () => {
  const errorCases: Array<{
    ...
    {
      status: 422,
      exception: UnprocessableRequestException,
      ...
      const response =
        status === 422
          ? {
              ...
              errors: [{ loc: ["body"], msg: "required", type: "value_error" }]
            }
        ...
        HttpResponse.json(
          {
            success: false,
            message:
              "Value error, cursor_after and cursor_before are mutually exclusive"
          },
          { status: 422 }
        )
      )
  ...
  it("serializes array query params as repeated keys", async () => {
    ...
    expect(captured.url?.searchParams.getAll("status")).toEqual([
      "active",
      "paused"
    ]);
  });
  ...
  it("filters out null/undefined values from array query params", async () => {
    ...
    await httpClient.get("/test-endpoint", {
      status: ["active", null, undefined, "paused"] as any
    });
  ...
  it("sends a patch request with a body", async () => {
    ...
    server.use(
      http.patch(
        `${TEST_CONFIG.baseUrl}/test-endpoint`,
        async ({ request }) => {
    ...
    const result = await httpClient.patch("/test-endpoint", {
      name: "updated-name"
    });
  ...
  it("GET can override baseRetryDelayMs", async () => {
    ...
    const result = await httpClient.get("/test-endpoint", undefined, {
      baseRetryDelayMs: 10
    });
  ...
  it("applies full jitter: delay = random(0, min(cap, base * 2^attempt))", async () => {
    ...
    await httpClient.get("/test-endpoint", undefined, {
