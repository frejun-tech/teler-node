...
describe("CallResourceManager (unit)", () => {
  ...
  describe("create", () => {
    ...
    it("propagates errors from the http layer", async () => {
      ...
      await expect(calls.create(CreateCallPayloadFixture())).rejects.toThrow(
        "Network error"
      );
    });
  });
  ...
  describe("retrieve", () => {
    ...
    it("propagates errors from the http layer", async () => {
      ...
      await expect(
        calls.retrieve("cs_01J5ABCDEFGHJKMNPQRSTVWXYZ")
  ...
  describe("listLegs", () => {
    it("gets the legs sub-resource path for a given call id", async () => {
      ...
      expect(http.get).toHaveBeenCalledWith(
        "/voice/calls/cs_01J5ABCDEFGHJKMNPQRSTVWXYZ/legs"
      );
    ...
    it("propagates errors from the http layer", async () => {
      ...
      await expect(
        calls.listLegs("cs_01J5ABCDEFGHJKMNPQRSTVWXYZ")
      ).rejects.toThrow("Network error");
    });
  });
});
