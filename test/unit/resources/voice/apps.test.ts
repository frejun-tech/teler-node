...
describe("AppResourceManager (unit)", () => {
  ...
  describe("create", () => {
    ...
    it("propagates errors from the http layer", async () => {
      ...
      await expect(apps.create(createVoiceAppPayloadFixture())).rejects.toThrow(
        "Network error"
      );
    });
  ...
  describe("list", () => {
    ...
    it("forwards status filter as an array correctly", async () => {
      ...
      expect(http.get).toHaveBeenCalledWith("/voice/apps", {
  ...
  describe("retrieve", () => {
    ...
    it("propagates errors from the http layer", async () => {
      ...
      await expect(
        apps.retrieve("va_01J5ABCDEFGHJKMNPQRSTVWXYZ")
  ...
  describe("delete", () => {
    ...
    it("propagates errors from the http layer", async () => {
      ...
      await expect(
        apps.delete("va_01J5ABCDEFGHJKMNPQRSTVWXYZ")
      ).rejects.toThrow("Network error");
    });
  });
  ...
  describe("listVirtualNumbers", () => {
    it("gets the correct sub-resource path with no params", async () => {
      ...
      const result = await apps.listVirtualNumbers(
        "va_01J5ABCDEFGHJKMNPQRSTVWXYZ"
      );
