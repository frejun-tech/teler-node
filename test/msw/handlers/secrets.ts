...
import { secretFixture, secretListFixture } from '../../support/fixtures/secrets';
...
export const secretHandlers = [
  ...
  http.patch(url('/secrets/:id'), async ({ params, request }) => {
    const body = (await request.json().catch(() => ({}))) as Partial<ReturnType<typeof secretFixture>>;
    return HttpResponse.json(secretFixture({ id: params.id as string, ...body }));
  }),

  http.delete(url('/secrets/:id'), () =>
    HttpResponse.json({ success: true, message: 'Secret deleted successfully.' })
  ),
];
